...
class NanImputer:
    ...
    def fit_transform(self, data: pd.DataFrame, nan_threshold : float = 1.0) -> pd.DataFrame:
        ...
        categorical_cols = data.select_dtypes(include=[object,"category","string",np.bool_]).map(lambda x: x if x is np.nan else str(x))
        numerical_cols = pd.DataFrame(self.numerical_imputer.fit_transform(numerical_cols),columns=numerical_cols.columns)
    ...
    def fit_transform_target(self, target: pd.Series) -> pd.Series:
        ...
        if np.issubdtype(target.dtype, np.number):
            target = pd.Series(self.numerical_imputer.fit_transform(np.transpose([target]))[:,0])
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        numerical_cols = data.select_dtypes(include=[np.number])
        categorical_cols = data.select_dtypes(include=[object,"category","string",np.bool_]).map(lambda x: x if x is np.nan else str(x))
        numerical_cols = pd.DataFrame(self.numerical_imputer.transform(numerical_cols),columns=numerical_cols.columns)
    ...
    def transform_target(self, target: pd.Series) -> pd.Series:
        if target.isin([0,1]).all():
            target  = target.astype(bool)
        if np.issubdtype(target.dtype, np.number):
            target = pd.Series(self.numerical_imputer.transform(np.transpose([target]))[:,0])
        elif np.issubdtype(target.dtype, np.object) or np.issubdtype(target.dtype, np.bool_):
            target = pd.Series(self.categorical_imputer.transform(np.transpose([target]))[:,0])
        return target
