#
# Taken from https://github.com/kheyer/OTDD
#
...
class ArrayDataset():
    ...
    def get_sample_idx(self, sample_size):
        sample_idxs = np.random.choice(
            np.arange(self.features.shape[0]), sample_size, replace=False)
...
distance function. Exercise caution in using Gaussian approximations.
    ...
    def gaussian_distance_from_stats(self, mu_x, sigma_x, mu_y, sigma_y, eps=1e-6):
        ...
        cost = gaussian_distance_from_stats(
            mu_x, sigma_x, mu_y, sigma_y, eps=1e-6)
...
class CostFunction():
    ...
    def distance(self, x_vals, y_vals, max_iter=None, mask_diagonal=False):
        ...
        cost, coupling = self.cost_function(
            x_weights, y_weights, M_dist, max_iter)
    ...
    def label_distances(self, x_vals, y_vals, x_labels, y_labels, max_iter=None, gaussian=False):
        ...
        for i, c1 in enumerate(class_x):
            ...
            for j, c2 in enumerate(class_y):
                ...
                if gaussian:
                    cost = self.distance_function.gaussian_distance_from_stats(
                        mu_xs[i], cov_xs[i], mu_ys[j], cov_ys[j])
                else:
                    cost, coupling, M_dist = self.distance(
                        sample_x, sample_y, max_iter=max_iter)
    ...
    def distance_with_labels(self, x_vals, y_vals, x_labels, y_labels, max_iter=None,
        ...
        if not gaussian_data_distance:
            ...
            dz = get_class_matrix(x_labels, y_labels,
                                  class_distances, class_x_dict, class_y_dict)
    ...
    def bootstrap_distance(self, num_iterations, dataset_x, sample_size_x,
        ...
        for i in range(num_iterations):
            ...
            if gaussian:
                cost = self.distance_function.gaussian_distance(
                    sample_x, sample_y)
            else:
                cost, coupling, M_dist = self.distance(
                    sample_x, sample_y, max_iter=max_iter)
    ...
    def bootstrap_label_distance(self, num_iterations, dataset_x, sample_size_x,
        ...
        for i in range(num_iterations):
            ...
            sample_x, label_x = self.filter_labels(
                sample_x, label_x, min_labelcount)
            sample_y, label_y = self.filter_labels(
                sample_y, label_y, min_labelcount)

            M_dist = self.distance_function(sample_x, sample_y, mask_diagonal)

            dz = get_class_matrix(
                label_x, label_y, class_distances, class_x_dict, class_y_dict)
            ...
            cost, coupling = self.cost_function(
                None, None, OTDD_matrix, max_iter)
...
class EarthMoversCost(CostFunction):
    ...
    def cost_function(self, x_weights, y_weights, M_dist, max_iter):
        ...
        output = ot.emd2(x_weights, y_weights, M_dist,
                         numItermax=max_iter, return_matrix=True)
...
class RobustOTCost(CostFunction):
    ...
    def cost_function(self, x_weights, y_weights, M_dist, max_iter):
        ...
        constraints = self.get_constraints(
            P, u, v, a_tilde, b_tilde, x_weights, y_weights)
...
def gaussian_distance_from_stats(mu_x, sigma_x, mu_y, sigma_y, eps=1e-6):
    ...
    if np.allclose(sigma_x, sigma_y) and np.allclose(mu_x, mu_y):
        ...
        cost = diff.dot(diff) + np.trace(sigma_x) + \
